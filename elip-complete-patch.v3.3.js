...
/*! elip-complete-patch.v3.3.js
   - Badge "N° corrente" (#pvCurrent) in header
   - Salva: fallback locale anche se saveToSupabase risponde senza numero
   - Post save: forza refresh archivio (counters) e mostra numero giusto
*/
(function(){
  'use strict';
  ...
  function ensureHeaderNodes(){
    ...
    var cur=q('#pvCurrent'); if(!cur){ cur=document.createElement('span'); cur.id='pvCurrent'; container.appendChild(cur); }
    return num;
  }
  function setHeaderNumero(n){
    ensureHeaderNodes();
    var v=(n && !/^ELP-/i.test(n) && n!=='—')?String(n).trim():'';
    AUTH_HEADER=v; allow=true;
    text(q('#quoteId'), v);
    var cur=q('#pvCurrent'); if(cur) cur.textContent = v ? ('N°: '+v) : '';
    setTimeout(function(){allow=false;},0);
  }
  function setHeaderCount(n){ ensureHeaderNodes(); var c=q('#pvCounter'); if(c) c.textContent='Totale preventivi: '+(n||0); }
  ...
  // Data accettazione pallino
  function ensureAccDot(){
    var input=q('#dataAcc'); if(!input) return;
    var lab=input.previousElementSibling; if(!lab) return;
    ...
    if(!dot){
      ...
      dot.style.width='14px'; dot.style.height='14px'; dot.style.borderRadius='50%'; dot.style.border='2px solid #dc3545'; dot.style.display='inline-block'; dot.style.marginLeft='8px'; dot.style.verticalAlign='middle'; dot.style.cursor='pointer';
      lab.appendChild(dot);
    ...
    input.addEventListener('change', function(){ var c=getCurrent()||{}; c.dataAcc=input.value||''; setCurrent(c); updateAccUI(); });
  }
  function updateAccUI(){
    ...
    var d=q('#accDot'); if(d){ d.style.borderColor = has?'#198754':'#dc3545'; d.style.background = has?'#198754':'#fff'; }
    // riflette anche nel badge corrente
    if(c.id && c.id!=='—') setHeaderNumero(c.id);
  }

  // ---- SAVE: mutex + fallback se errore OPPURE se non torna un numero
    ...
    var tryWrap=function(){
      ...
      window.saveToSupabase=async function(){
        ...
        try{
          ...
          var cur=getCurrent()||{};
          if(!numero){
            // fallback locale: non pericoloso (non duplica su backend)
            if(!cur.id || cur.id==='—'){ cur.id='PV-LOCAL-'+Date.now(); }
            numero=cur.id;
          ...
          cur.id=numero; setCurrent(cur); setHeaderNumero(numero);
          // conferma
          showConfirm(numero);
          // refresh archivio/counters
          if (typeof renderArchive==='function') renderArchive();
          return res;
        } catch(e){
          // anche in caso di exception → fallback locale
          try{
            var cur=getCurrent()||{};
            if(!cur.id || cur.id==='—'){ cur.id='PV-LOCAL-'+Date.now(); }
            var numero=cur.id;
            ...
            setArchiveLocal(arch);
            setCurrent(cur); setHeaderNumero(numero);
            showConfirm(numero);
            if (typeof renderArchive==='function') renderArchive();
          }catch(_){}
          throw e;
  ...
  function showConfirm(numero){
    ...
    var t=q('#saveConfirmText'); if(t) t.textContent='Preventivo salvato con successo. N°: '+(numero||'-');
    if(window.bootstrap){ new bootstrap.Modal(q('#saveConfirmModal')).show(); } else alert('Preventivo salvato. N°: '+(numero||'-'));
  }

  // Hook fetch: sync header + refresh contatore
  ;(function(){
