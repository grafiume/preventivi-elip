...
function renderLines(){
  ...
  const body=qs('#linesBody'); if(!body) return; body.innerHTML='';
...
function recalc(){
  ...
  const impEl=qs('#imponibile'), ivaEl=qs('#iva'), totEl=qs('#totale');
  if(impEl) impEl.textContent=EURO(imponibile);
  if(ivaEl) ivaEl.textContent=EURO(iva);
  if(totEl) totEl.textContent=EURO(total);
  setCurrent(cur);
}
...
function renderImages(){
  const cur=appInitData();
  const wrap=qs('#imgPreview'); if(!wrap) return; wrap.innerHTML='';
  ...
  wrap.onclick = (e)=>{
    ...
    if(img){
      ...
      if (window.bootstrap && bootstrap.Modal) { const modal = new bootstrap.Modal(document.getElementById('imgModal')); modal.show(); }
      else window.open(cur.images[idx],'_blank');
    }
  };
}
...
async function previewPDF(type){
  const doc=await buildPDF(type); if(!doc) return;
  const blob=doc.output('blob'); const url=URL.createObjectURL(blob);
  const f=qs('#pdfFrame'); if(f) f.src=url;
  const a=qs('#btnDownload'); if(a){ a.href=url; a.download=(appInitData().id)+'-'+(type==='dett'?'dettaglio':'totale')+'.pdf'; }
  if (window.bootstrap && bootstrap.Modal) { const modal = new bootstrap.Modal(document.getElementById('pdfModal')); modal.show(); }
...
function jpgCover(){
  ...
  g.fillText('Totale (IVA 22%): '+(qs('#totale')?qs('#totale').textContent:'€ 0,00'), 40, 320);
  g.font='bold 24px Arial'; g.fillText('Voci incluse', 40, 370);
  g.font='20px Arial'; let y=410; const step=28, maxRows=12;
  appInitData().lines.slice(0,maxRows).forEach(r=>{ g.fillText(`${r.code||''} — ${r.desc||''}`, 40, y); y+=step; });
  const url=c.toDataURL('image/jpeg',0.92); const link=qs('#btnJPG'); if(link){ link.href=url; link.download=appInitData().id+'-anteprima.jpg'; }
}
function shareMail(){
  const cur=appInitData();
...
function shareWA(){
  ...
  const msg = encodeURIComponent(`ELIP Tagliente — Preventivo ${cur.id}%0ACliente: ${cur.cliente||'-'}%0ATotale: ${(qs('#totale')?qs('#totale').textContent:'€ 0,00')}%0A%0AVoci:%0A`)+lines;
...
function fillForm(){
  const cur=appInitData();
  const el=qs('#quoteId'); if(el) el.textContent=cur.id;
  ['cliente','articolo','ddt','telefono','email','dataInvio','dataAcc','dataScad','note'].forEach(id=>{ const x=qs('#'+id); if(x) x.value = cur[id]||''; });
}

function bindAll(){
  qs('#btnNew')?.addEventListener('click', newQuote);
  qs('#btnSave')?.addEventListener('click', () => window.saveToSupabase && window.saveToSupabase(false));
  qs('#btnArchive')?.addEventListener('click', () => window.saveToSupabase && window.saveToSupabase(true));
  ...
  ['cliente','articolo','ddt','telefono','email','dataInvio','dataAcc','dataScad','note'].forEach(id=>{ const x=qs('#'+id); if(x){ x.addEventListener('input', recalc); x.addEventListener('change', recalc); }});
  qs('#imgInput')?.addEventListener('change', e=> e.target.files.length && handleImages(e.target.files));
  qs('#btnReloadArch')?.addEventListener('click', () => window.loadArchiveSupabase ? window.loadArchiveSupabase().then(renderArchiveLocal) : renderArchiveLocal());
...
function init(){
  ...
  (window.loadArchiveSupabase ? window.loadArchiveSupabase() : Promise.resolve([])).then(()=>{ renderArchiveLocal(); if(window.subscribeRealtime) window.subscribeRealtime(); });
}
