// UI logic v6.1-SB (no autosave). Works with app-supabase.js for cloud sync.
...
function addLine(line){ const cur=bootstrap(); cur.lines.push(line); setCurrent(cur); renderLines(); recalc(); }
...
function onLineClick(e){
  ...
  if(btnDel){ const idx=+btnDel.getAttribute('data-del'); const cur=bootstrap(); cur.lines.splice(idx,1); setCurrent(cur); renderLines(); recalc(); return; }
...
function renderImages(){
  ...
  wrap.onclick = (e)=>{
    ...
    if(b){ const i=+b.getAttribute('data-delimg'); const cur=bootstrap(); cur.images.splice(i,1); setCurrent(cur); renderImages(); return; }
    const img=e.target.closest('img[data-zoom]'); 
    if(img){
      const idx=+img.getAttribute('data-zoom'); const cur=bootstrap();
...
async function buildPDF(type){
  ...
  const cur=bootstrap(), doc=new jsPDF({unit:'pt',format:'a4'}), pad=40;
  ...
  if(type==='dett'){
    ...
    doc.autoTable({startY:230, head:[['Cod','Descrizione','Q.tà','Prezzo','Totale']],
      ...
      columnStyles:{0:{cellWidth:55},1:{cellWidth:260},2:{cellWidth:60,halign:'right'},3:{cellWidth:85,halign:'right'},4:{cellWidth:95,halign:'right'}}});
  }else{
...
async function previewPDF(type){
  ...
  qs('#pdfFrame').src=url;
  const a=qs('#btnDownload'); a.href=url; a.download=(bootstrap().id)+'-'+(type==='dett'?'dettaglio':'totale')+'.pdf';
...
function jpgCover(){
  ...
  bootstrap().lines.slice(0,maxRows).forEach(r=>{ g.fillText(`${r.code||''} — ${r.desc||''}`, 40, y); y+=step; });
...
BIC/SWIFT CODE: BPBAIT3B`;
  ...
  <body style="font-family:Arial,Helvetica,sans-serif;line-height:1.6;color:#222">
    <div style="max-width:860px;margin:24px auto">
      <div style="text-align:center;margin-bottom:16px">
        <img src="logo-elip.jpg" style="height:70px">
      </div>
...
/* Buttons */
function newQuote(){ localStorage.removeItem('elip_current'); const cur=bootstrap(); qs('#quoteId').textContent=cur.id; renderLines(); renderImages(); recalc(); }
function bindAll(){
...
/* Archivio rendering (usa cache locale dello scaricato da Supabase) */
...
function init(){
  ...
  // primo caricamento da Supabase
  loadArchiveSupabase().then(()=>{
    renderArchiveLocal();
    subscribeRealtime();
  });
}
