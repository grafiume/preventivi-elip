...
/*! elip-complete-patch.v2.8.js (clean)
   - Header: mostra SOLO il numero (vuoto finché non salvato)
   - "Nuovo" forza passaggio a Editor con pagina pulita
   - Archivio: filtri eleganti con contatori nei bottoni (Tutti/Accettati/Non accettati/Chiusi)
   - "Chiusi" = Accettazione OK + Avanzamento >= 100%
   - Nessun regex literal: uso RegExp per compatibilità (fix "Invalid regular expression flags")
   - Hook su saveToSupabase e fetch per sincronizzare l'header
*/
(function(){
  'use strict';
  ...
  function setHeaderNumero(numero){
    ...
    text(el, AUTH_HEADER);
    setTimeout(function(){ allowExternal = false; }, 0);
  }
  var headerObs = new MutationObserver(function(muts){
    muts.forEach(function(m){
      var t = m.target;
      if (t && t.id === 'quoteId' && !allowExternal){
  ...
  // patch fillForm: header + okPill se presente
  (function(){
    if (!window.fillForm) return;
    var _orig = window.fillForm;
    window.fillForm = function(){
      var r;
      try { r = _orig.apply(this, arguments); } catch(e){ /* evita crash */ }
      try{
        var c=getCurrent()||{};
