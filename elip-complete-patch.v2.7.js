...
/*! elip-complete-patch.v2.7.js
   - Header: solo numero (vuoto prima del salvataggio)
   - Filtri uniformi ed eleganti (btn-filter). Contatori dentro ai bottoni.
   - Rimosso accCounters (non scritto più).
*/
(function(){
  'use strict';
  ...
  // Patch fillForm/recalc
  (function(){
    if(!window.fillForm) return;
    var _f=window.fillForm;
    window.fillForm=function(){ var r=_f.apply(this,arguments); try{ var c=getCurrent()||{}; setHeaderNumero(c.id); var ok=q('#okPill'); if(ok){ var has=!!(c.dataAcc&&String(c.dataAcc).trim()); ok.classList.toggle('acc-ok',has); ok.classList.toggle('acc-no',!has); ok.textContent= has?'● OK':'● NO'; } }catch(e){} return r; };
  })();
  (function(){
    if(!window.recalc) return;
    var _r=window.recalc;
    window.recalc=function(){ var r=_r.apply(this,arguments); try{ var c=getCurrent()||{}; var impEl=q('#imponibile'),ivaEl=q('#iva'),totEl=q('#totale'); if(!(impEl&&ivaEl&&totEl)){ var lines=c.lines||[]; var imp=lines.reduce((s,rr)=>s+(+rr.qty||0)*(+rr.price||0),0); var iva=imp*0.22, tot=imp+iva; if(impEl)impEl.textContent=EURO(imp); if(ivaEl)ivaEl.textContent=EURO(iva); if(totEl)totEl.textContent=EURO(tot); } var ok=q('#okPill'); if(ok){ var has=false; var i=q('#dataAcc'); if(i&&i.value&&String(i.value).trim()) has=true; else if(c.dataAcc&&String(c.dataAcc).trim()) has=true; ok.classList.toggle('acc-ok',has); ok.classList.toggle('acc-no',!has); ok.textContent=has?'● OK':'● NO'; } }catch(e){} return r; };
  })();
  ...
  async function renderArchive(){
    ...
    filtered.forEach(function(rec){
      ...
      var dot=(pct===100)?'<span class="progress-dot" style="color:var(--elip-green)">●</span>':(pct>=50?'<span class="progress-dot" style="color:#ffc107">●</span>':'<span class="progress-dot" style="color:var(--elip-red)">●</span>');
