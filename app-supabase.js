// Supabase integration v6.4-SB (no UI config)
...
window.saveToSupabase = async function (archiveAfter) {
  ...
  if (!cli) { alert('Supabase non configurato'); return; }
  ...
  if (existing.error && existing.error.code !== 'PGRST116') { console.error('[Supabase] Read error:', existing.error); return; }

  if (existing.data) {
    const upd = await cli.from('preventivi').update(payload).eq('id', existing.data.id);
    if (upd.error) { console.error('[Supabase] Update error:', upd.error); alert("Errore aggiornamento: " + upd.error.message); return; }
  } else {
    const ins = await cli.from('preventivi').insert(payload);
    if (ins.error) { console.error('[Supabase] Insert error:', ins.error); alert("Errore inserimento: " + ins.error.message + "\\nEsegui lo schema.sql in Supabase per creare le colonne 'linee' e 'images' (JSONB)."); return; }
  }
...
window.loadArchiveSupabase = async function () {
  ...
  if (error) { console.error('[Supabase] Errore caricamento archivio:', error); localStorage.setItem('elip_archive','[]'); return []; }
  localStorage.setItem('elip_archive', JSON.stringify(data || []));
  return data || [];
};

window.subscribeRealtime = function () {
  const cli = supaClient(); if (!cli) return;
