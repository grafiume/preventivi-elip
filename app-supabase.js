...
async function uploadImagesIfNeeded(idPreventivo, images){
  ...
  for(const src of (images||[])){
    if(typeof src === 'string' && src.startsWith('http')) { uploaded.push(src); continue; }
    ...
    const { error } = await cli.storage.from(window.supabaseConfig.bucket).upload(key, blob, { upsert: false });
...
async function saveToSupabase(archiveAfter){
  ...
  cur.images = uploadedUrls; localStorage.setItem('elip_current', JSON.stringify(cur));
  ...
  await loadArchiveSupabase(); renderArchiveLocal(); toastSaved();
