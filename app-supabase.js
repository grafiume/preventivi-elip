...
(function(){
  ...
  function supa() {
    ...
    if (!window.supabase || !window.supabase.createClient) throw new Error('Supabase library non caricata');
  ...
  async function saveToSupabase(archiveAfter){
    ...
    for (const f of queue) { try { await uploadPhoto(f, cur.id); } catch(e){ console.warn('[uploadPhoto]', e); } }
